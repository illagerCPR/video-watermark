...
- **批量处理**：多视频一次生成，逐文件进度（GUI 对话框与 TUI 批量屏共用同一后端 `app/core/batch.py`，v0.5.6 起）；完成日志标注每个文件实际使用的编码器（v0.5.7 起）
...
- **报"硬件编码器不可用"但 TUI 或「检测」显示正常**：检查 GUI「输出设置 → ffmpeg 二进制来源」——若选了**「内置」**，Linux 内置版没有编译任何硬件编码器（改回「自动（推荐）」即可）；v0.5.7 起该报错末尾会显示当前实际使用的二进制与来源，可直接对照。另外探测结果按二进制指纹缓存，**瞬时探测失败（如 NVENC 会话偶发建不起来）不会被记住**（v0.5.7 起空结果不落盘，下次启动自动重测）；批量并行写入缓存已改原子写，不再可能交错损坏。
