...
- **`app/tui.py`（TUI，v0.5.0）**：Textual 全屏应用——全字段表单（collect_config 抛 ConfigError 带[字段名]）、fill_from_config 回填、collect_export_params（crf/preset/scale/hw_*/parallel）、ExportScreen（ProgressBar+取消）、PreviewScreen（半块像素）、BatchScreen（v0.5.6 批量：文件/目录入队+ListView+并行数+文件级/帧级进度+RichLog+Esc 取消）。**TUI 屏内两个必踩坑（v0.5.6）**：①挂在 Screen 上的 worker 方法调度回 UI 线程必须 `self.app.call_from_thread(...)`——Screen 没有 `call_from_thread`，误用 `self.call_from_thread` 抛 AttributeError 且会被回调里的 except 静默吞掉（表现为后台任务正常完成但 UI 零更新、无法收尾）；②屏实例自有状态布尔**禁用 `_running` 这个名字**——与 Textual `MessagePump._running`（消息泵启动即置 True）撞名，守卫永远为真；用 `_batch_active` 之类的名字。**布局陷阱：`#form` 必须 `height: 1fr`（滚动视口），用 auto 会溢出屏幕且无法滚动**。**行容器陷阱（v0.5.1）**：滚动容器内 `Horizontal` 默认 `height: 1fr` 会塌缩成 1 行（按钮 3 行画不下→滚到底也看不到最后一行），必须显式 `#form Horizontal { height: auto }`；`Input` 默认 `width: 100%`，与按钮同处一行会把按钮挤出 `overflow: hidden` 的行外，行内须改 `width: 1fr`。
...
- **`app/core/batch.py`（批量核心，v0.5.6）**：GUI/TUI 共用批量后端——`scan_videos`（文件直通/目录递归扫 VIDEO_EXTS）、`plan_jobs`（`原名_水印.扩展名`）、`run_batch(jobs, cfg, export_params, batch_parallel, *, progress_q, frame_cb, file_cb, cancel_event)`（串行=逐文件 process 可中断当前文件；并行=ProcessPoolExecutor **有界提交**（保持 ≤parallel 个在途，取消后不再提交）+ Manager().Queue 帧进度经转发线程回调 frame_cb；返回 `{"total","ok","fail","cancelled"}`，取消跳过的文件不计 ok/fail）。完成消息带实际编码器（v0.5.7：`完成：xxx（h264_nvenc）`，并行模式靠 `_run_one` 返回 process() 的 stats 跨进程回传）。`export_params` 不透传 parallel（批量下帧流水线保持 auto，与 GUI 批量一致）。GUI `BatchWorker` 与 TUI `BatchScreen._batch_worker` 都只是回调桥接，执行逻辑唯一来源在此。**Manager 及其 Queue 必须在 UI 主线程创建**（Windows 非主线程建 Queue 会 WinError 5），创建失败传 None = 并行无帧进度不阻断。
...
- 探测结果缓存路径：Windows `%APPDATA%/VideoWatermark/`；Linux/macOS `~/.config/VideoWatermark/`（XDG_CONFIG_HOME 优先）。**写入纪律（v0.5.7）**：原子写（tmp + `os.replace`——批量并行子进程会并发写同一文件）+ **空结果不落盘**（瞬时探测失败如 NVENC 会话偶发建不起来，若持久化会在此后所有会话持续"未检测到"；空结果只留会话内 lru，下次会话重测）。显式硬件编码器不可用的报错附当前二进制及来源（`ffbin.info()`）。
...
- `smoke_test.py` 轨迹坐标/文字图片渲染逻辑；`verify_ffbin.py` ffbin 解析层专项（internal/显式路径/环境变量优先级/联动，v0.3.2 起）；`tui_test.py` TUI 全流程专项（Pilot 无终端自动化：表单往返/校验/配置预载保存/预览屏/导出取消，v0.5.0 起；第 9 节布局回归：横向溢出/行内越界/底部按钮可达，v0.5.1 起）；`tui_batch_test.py` TUI 批量专项（v0.5.6：core 层 scan_videos/plan_jobs/run_batch 取消语义 + Pilot 端到端串行/并行/取消/布局回归）；`verify_step1.py` 像素级成品验证（水印差异 + 轨迹质心 vs `position_at`）；`gui_smoke.py` GUI 离屏冒烟（第 8 节长路径检测布局回归，v0.5.1 起）；`gui_export_test.py`/`step3_export_test.py`/`step4_batch_test.py` 端到端导出/编码参数/批量；`verify_time_range.py` 时间范围；`verify_audio.py` 音频保留；`verify_hw.py` 硬件加速专项（探测 + 各硬件编码器实跑 + 回退 + 硬解 + 音频，无硬件编码器时 GPU 项自动 SKIP）；`verify_pipeline.py` 并行流水线专项（串行/并行字节级一致 + GPU/移动/硬解组合，GPU 断言同样环境感知）；`step1_demo.py` 生成样例输出到 `outputs/`。
