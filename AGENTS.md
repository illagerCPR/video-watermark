...
- **Linux 版内置 ffmpeg 未编译任何硬件编码器**（`-encoders` 中无 `*_nvenc/_qsv/_amf/_d3d12va/_mf`，v0.3.0 实测）：Linux 上 `detect_encoders()` 恒返回 `{}`，`auto` 静默回退 libx264，显式指定按设计报错——**属预期行为，勿当 bug 修**。探测流程先比对 `-encoders` 名单再实测，未编译候选零成本跳过，注册表无需按平台裁剪。
...
- **Linux 平台预期差异（勿当 bug）**：`verify_hw.py` 的"探测含 nvenc"与"至少一个硬件编码器实跑"、`verify_pipeline.py` 的 GPU 编码断言在 Linux 会 FAIL——根因是 Linux 版内置 ffmpeg 无硬件编码器，其余全部路径应通过。
